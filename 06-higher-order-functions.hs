...
higherOrderSequenceApplication f op a b z
  | a > b = z
  | otherwise = op a `f` higherOrderSequenceApplication f op (a + 1) b z

-- Define a factorial method using the higherOrderSequenceAppliction
hoFactorial :: Int -> Int
hoFactorial x = higherOrderSequenceApplication (*) id 1 x 1
